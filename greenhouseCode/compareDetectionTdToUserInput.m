...
interFrameInterval =  10000;%mode(diff(surfaceSamplingTimeArray));
...
%try ShowParamters.xMax = xMax;ShowParamters.yMax = yMax;ShowParamters.tMax = tdPosClean.ts(end);  end
%try showHypTdIn3d(downSampleTd(TD,10),ShowParamters); end
%try showMeanLinesIn3d(userInputCellArray); end
%drawnow
...
%for densityToTest = meanDensity
    ...
    if EMPTY_USER_INPUT_FLAG
        ...
        tpPerDist    =  fpPerDist*0+1;
        fnPerDist    =  fpPerDist*0+0;
    else
        
        for iDist = 1:nPlausibleDistanceThreshold
            tpPerDist(iDist,1) = nanmean(mat2vec(tpPerDistPerFramePerObj(iDist,:,:)));
            fnPerDist(iDist,1) = nanmean(mat2vec(fnPerDistPerFramePerObj(iDist,:,:)));
        end
        %tpPerDist                =   nanmean(tpPerDistPerObj,2);
        %fnPerDist                =   nanmean(fnPerDistPerObj,2);
    end
...
try
    if any(isnan([tnPerDist  tpPerDist fnPerDist fpPerDist  ]))
    end
catch
end
if nObj >1
