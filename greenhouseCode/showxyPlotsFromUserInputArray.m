function showxyPlotsFromUserInputArray(userInputArray,varargin)
interpObjLineWidth = 1;
SHOW_RAW_USER_INPUT_MARKINGS = 1;
userMarkingMarkerSize = 9;
nFrame = size(userInputArray,1);
if nargin>2
    interpedUserInputArray = varargin{2};
    if isempty(interpedUserInputArray)
...
xMax = max(real(userInputArray(:)));
if isnan(xMax)
    xMax = 0;
end
yMax = max(imag(userInputArray(:)));
