...
%%
load("recordings/newdata_goodrecording.mat")
...
%% Load all variables
% load('recordings/newData_goodRecording_onlylabels_FEAST.mat')
...
%         P_FdSimple(xd,yd,winnerNeuron) = p;
        ...
        if isinf(T_FdSimple(xd,yd,winnerNeuron))
            ...
            X_original(idx,:) = S_Fd_featureContext(:)'; % X matrix. to later be split into Xtrain and Xtest
            
            if TD.c(idx) == 'n'
                labels(idx,1) = 0;  %Y vector to later be split into Ytrain and Ytest
            elseif  TD.c(idx) == 'f'
                labels(idx,1) = 1;
            end
        end
...
Y = single(labels(1:nEventsToSkip:end));
...
Ytest(:,2) = ~(Ytest(:,1));
toc
%% Load Xtrain, Xtest, Ytrain and Ytest
load("recordings/training_data/dataXY.mat")
%% Classification

ClassifierParameters.ELM_hiddenLayerSizes           = 1000;
...
ClassifierParameters.SHOW_CLASSIFIER_OUTPUT_PLOT    = 0;
 ...
 for i=1:200
    [rootMeanSquareErrorPerformanceTrain(i),rootMeanSquareErrorPerformanceTest(i),rootMeanSquareErrorPerformanceTrainRBTF(i),rootMeanSquareErrorPerformanceTestRBTF(i)]=ELM_ComputeRMSE(Xtrain,Ytrain,Xtest,Ytest,i);
    fig = figure(6767);
    subplot(2,1,1)
 ...
 for i=1:200
     ...
     %     subplot(2,1,1)
     plot(rootMeanSquareErrorPerformanceTrain,'g','LineWidth',2);hold on
     plot(rootMeanSquareErrorPerformanceTest,'r','LineWidth',2);hold on
     plot(rootMeanSquareErrorPerformanceTrainRBTF,'b','LineWidth',2);hold on
     plot(rootMeanSquareErrorPerformanceTestRBTF,'y','LineWidth',2);
     xlabel("Hidden Nodes")
     ylabel("RMSE")
     legend("Train - Sigmoid", "Test - Sigmoid", "Train - RBF", "Test - RBF")
     grid on
     title(append("Epoch: " + num2str(i)))
