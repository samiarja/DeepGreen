function [TD,varargout] = cleanUpTd(inputTD,varargin)
%%% Example Usage --------------------------------------------------------------
% [TD] = cleanUpTd(TD)
...
TD = inputTD;
if nargin>1
    givenStartEndTimeArray = varargin{1};
    if  numel(givenStartEndTimeArray)~=2
        initialStartTimeIn_uSec = originalStartTimeIn_uSec;% default
        initialEndTimeIn_uSec   = originalEndTimeIn_uSec;% default
    else
        givenStartTimeIn_uSec = givenStartEndTimeArray(1);
        givenEndTimeIn_uSec   = givenStartEndTimeArray(2);
        if givenStartTimeIn_uSec<originalStartTimeIn_uSec  || isnan(givenStartTimeIn_uSec)
            initialStartTimeIn_uSec = originalStartTimeIn_uSec;% default
        else
            initialStartTimeIn_uSec =  givenStartTimeIn_uSec;
        end
        if givenEndTimeIn_uSec<originalEndTimeIn_uSec  || isnan(givenEndTimeIn_uSec)
            initialEndTimeIn_uSec = originalEndTimeIn_uSec;% default
        else
...
if nargin>2
    prctHotPixelsToRemove = varargin{2};
...
PRE_FILTER_TD = 0;
if nargin>3
    preFilterTimeIn_uSec  = varargin{3};
    if ~isempty(preFilterTimeIn_uSec)
        PRE_FILTER_TD = 1;
    end
end
if nargin>4
    preFilterRadius  = varargin{4};
...
if nargin>5
    SHOW_CLEANED_UP_TD_RESULTS = varargin{5};
...
if 1
    ...
    while ~isempty(negTimeDiffFirstIndexArray)
        ...
        diff = TD.ts(2:(nEventsLeftInTd)) - TD.ts(1:(nEventsLeftInTd-1));
        diffNegRect = min(diff,0);
...
[xMax,yMax] = findxyMaxFromTd(TD,xyMaxThreshold);
...
if ~isempty(prctHotPixelsToRemove)
    if prctHotPixelsToRemove ~= 0
        ...
        for indexOfIndex = 1:nEventsToRemoveDueToBadTime
            ...
            figure(43432);
            histogram(initialEventCountPerPixel(:));
            set(gca,'yscale','log');
            set(gca,'xscale','log');
            grid on;
        ...
        for iTd = 1:nEventsLeftInTd
            if TD.p(iTd) == 1
                x = TD.x(iTd);
                y = TD.y(iTd);
                cleanedEventCountPerPixelPos(x,y) = cleanedEventCountPerPixelPos(x,y)+1;
            elseif TD.p(iTd) == -1
                x = TD.x(iTd);
                y = TD.y(iTd);
